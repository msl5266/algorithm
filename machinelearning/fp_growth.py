...
class node:
    def __init__(self, item, count, parent): # 将节点之间的链接信息存储到项表头中，后续可遍历项头表添加该属性
...
class FP:
    ...
    def init_param(self, data):
        self.N = len(data)
        self.minsup_sum = math.ceil(self.minsup*self.N)
        self.get_fre_one_itemset(data)
        self.build_tree(data)
        return

    def get_fre_one_itemset(self, data):
        #获取频繁一项集，并排序，第一次扫描数据集
        c = Counter()
        for t in data:
            c += Counter(t)
        for key, val in c.items():
            if val >= self.minsup_sum:
    ...
    def insert_item(self, parent, item):
        ...
        for child_id in children:
            ...
            self.max_node_id += 1
            next_node_id = copy.copy(self.max_node_id) # 注意self_max_node_id是可变的，引用时需要copy
    ...
    def extract_suffix_set(self, condition_tree, suffix_items):
        ...
        for item, sup in item_sup_dict.items():
            if sup >= self.minsup_sum: #若条件fp树中某个项是频繁的，则添加到后缀中
    ...
    def get_fre_set(self, data):
        ...
        pre_tree = copy.deepcopy(self.tree) #pre_tree是尚未去除任何后缀的前驱，若其叶节点的项有多种，则可以形成多种条件fp树
    ...
    def bfs_search(self, pre_tree, suffix_items_list, suffix_items_id_list):
        ...
        while len(q)>0:
            ...
            for suffix_items, suffix_items_id in zip(param_tuple[1], param_tuple[2]):
                condition_tree = self.get_condition_tree(pre_tree, suffix_items_id)
                new_suffix_items_list, new_suffix_items_id_list = self.extract_suffix_set(condition_tree, suffix_items)
                if new_suffix_items_list: #如果后缀有新的项添加进来，则继续深度搜索
                    self.bfs_search(condition_tree, new_suffix_items_list, new_suffix_items_id_list)
        return
