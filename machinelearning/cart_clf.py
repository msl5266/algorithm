"""
CART分类树，是一颗二叉树，以某个特征以及该特征对应的一个值为节点，故相对于ID3算法，最大的不同就是特征可以使用多次
"""
...
class CART_CLF:
    def __init__(self, epsilon=1e-3,min_sample=1):
        ...
        self.min_sample = min_sample #叶节点含有最少样本量
        self.tree = None
    ...
    def fit(self, X_data, y_data):
        # 训练模型，CART分类树与ID3最大的不同是，CART建立的是二叉树，每个节点是特征值及其对应的某个值组成的元组
        # 特征可以多次使用
        splits_set = []
        for fea in range(X_data.shape[1]):
            ...
            if unique_vals.shape[0] < 2:
                continue
            elif unique_vals.shape[0] == 2: #若特征取值只有两个，则只有一个切分点，非此即彼
