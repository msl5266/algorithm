...
class heap:
    ...
    def _move_down(self, item):
        ...
        pos = 0
        left_pos = 2*pos+1 # 左树为交换节点
        while left_pos<len(self.h):
            right_pos = 2*pos+2
            if right_pos<len(self.h) and self.h[right_pos][1]>self.h[left_pos][1]:
                left_pos = right_pos
            if item[1] < self.h[left_pos][1]:
                self.h[pos], self.h[left_pos] = self.h[left_pos], self.h[pos]
                pos = left_pos
                left_pos = 2*pos+1
                continue
            else:
