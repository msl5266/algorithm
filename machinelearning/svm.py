"""
支持向量机
学习方法：不等式约束的组优化问题，求解凸二次规划的最优化算法，本程序采用最小最优化算法（SOM）
"""
import numpy as np
import math
...
class SVM:
    def __init__(self, epsilon=1e-5, maxstep=500,C=1.0,kernel_option=True,gamma=None):
        ...
        self.gamma = gamma #高斯核函数

        self.kernel_arr = None # n*n存储核内积
        self.X = None #训练数据集
        self.y = None #类标记值，是计算w，b的参数，故存在于模型中
        self.alpha_arr = None #1*n存储拉格朗日乘子，每个样本对应一个拉格朗日乘子
        self.b = 0 #阈值b，初始化为0
    ...
    def update(self, ind1, ind2):
        ...
        #更新alpha1
        if abs(old_alpha2-new_alpha2)<self.epsilon*(old_alpha2+new_alpha2+self.epsilon):
            #若alpha2更新变化很小，则忽略本次更新
            return 0
    ...
    def satisfy_ktt(self,y,err,alpha):
        #在精度范围内是否满足ktt条件
        r = y*err
    ...
    def fit(self,X_data,y_data):
        ...
        # 整个训练过程需要在边界支持向量与所有样本之间进行切换搜索，以防止无法收敛
        entire_set = True
        step = 0
        change_pairs = 0
        while step<self.maxstep and (change_pairs>0 or entire_set): #当搜索全部样本，依然没有改变，则停止迭代
            step += 1
            change_pairs = 0
            if entire_set: #搜索整个样本集
                for ind1 in range(self.N):
                    if not self.satisfy_ktt(y_data[ind1], self.err_arr[ind1], self.alpha_arr[ind1]):
...
if __name__ == "__main__":
    ...
    y_data[inds] = -1
    from machinelearning.cross_validation import validate
