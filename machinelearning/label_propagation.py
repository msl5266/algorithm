"""
标签传播聚类算法，典型的半监督学习算法
核心思想:相似的数据应该有相同的标签，构建节点间的相似性矩阵（边的权重）
"""
import numpy as np
...
class LabelPropagation:
    ...
    def init_param(self, X_data, y_data):
        ...
        self.Y_clamp = self.Y[self.labeled_inds] #n*1
...
if __name__ == "__main__":
    ...
    n_samples = 100
    X, y = make_blobs(n_samples=n_samples, shuffle=False)
    outer, inner = 0, 1
    labels = -np.ones(n_samples)
    labels[0] = outer
    labels[-1] = inner
    LPA = LabelPropagation(maxstep=1000, kernel_option='knn', k=2, sigma=0.07)
    LPA.fit(X, labels)
    labels = LPA.labels
