...
class DBScan:
    ...
    def init_param(self, data):
        ...
        self.cluster_labels = np.zeros(self.data.shape[0]) # 将所有类标记设为0
        self.cal_pair_dis()
        return
    ...
    def fit(self):
        ...
        for p in core_points:
            if not self.cluster_labels[p]: # 若核心点未被标记，则建立新簇，族标记加1
                q.append(p)
                cluster_label += 1
                # 以当前核心点为出发点，采用广度优先算法进行簇的扩展，直至队列为空，则停止此次扩展
                while len(q) > 0:
                    p = q.pop()
                    self.cluster_labels[p] = cluster_label
                    for n in neighs[p]:
                        if not self.cluster_labels[n]: #领域内的点未归类，则加入该簇
                            self.cluster_labels[n] = cluster_label
                            if n in core_points: # 若领域内存在未标记的核心点，则依据该核心点继续扩展簇（一定要未标记，否则造成死循环）
                                q.appendleft(n)
