...
from data_helpers.data_base import TrainDataBase
...
class TrainData(TrainDataBase):
    def __init__(self, config):
        ...
        self._stop_word_path = os.path.join(os.path.abspath(os.path.dirname(os.getcwd())), config['stop_word']) if config['stop_word'] else None
    ...
    def remove_stop_word(self, inputs):
        ...
        if self._stop_word_path:
            with open(self._stop_word_path, 'r', encoding='utf-8') as fr:
    ...
    def gen_vocab(self, words, labels):
        """
        生成词汇，标签等映射表
        :param words:训练集所包含的单词
        :param labels:标签
        :return:
        """

        # 如果不是第一次处理，可以直接加载生成好的词汇表和词向量
        ...
        # 若vocab的长度小于设置的vocab_size,则选择vocab的长度作为真实的vocab_size
